...
## 回答要求
- 如果参考资料中有明确答案，请详细说明，并 **在正文中引用出处**（文件名、页码，与参考资料标注一致）
- 如果资料不够完整，请基于常识补充，但需说明哪些是推断内容
- 如涉及具体流程或制度，请按步骤清晰说明
- 适当使用"建议您"、"请注意"、"温馨提示"等友好表达
- **禁止** 引用参考资料中未出现的具体条款号、福利天数、审批节点或数字
...
3. 不要编造未出现的内容；忽略寒暄与重复。
4. 输出长度约 {max_chars} 字以内。"""

USER_MEMORY_VS_RAG_BOUNDARY_NOTE = """## 记忆与知识库分工
- **参考资料（RAG）** 中的制度、流程、条款为权威依据；制度类问题必须以参考资料为准。
- **用户相关信息** 仅为用户偏好或身份类说明，不得替代制度原文，不得当作公司规定引用。
- 对话摘要中的内容仅供理解上下文，不得当作正式制度条款。"""

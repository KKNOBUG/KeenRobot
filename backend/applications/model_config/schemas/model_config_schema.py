...
from backend.applications.model_config.services.secret_utils import (
    decrypt_api_key,
    mask_api_key,
)
...
class ModelConfigBase(BaseModel):
    ...
    temperature: Optional[float] = Field(default=None, ge=0, le=1, description="温度控制")
    max_tokens: Optional[int] = Field(default=None, ge=512, le=128000, description="限制单次回答的最大输出Token数")
    top_p: Optional[float] = Field(default=None, ge=0, le=1, description="核采样数(TOP-P)")
...
class ModelConfigCreate(ModelConfigBase):
    ...
    temperature: float = Field(default=0.7, ge=0, le=1, description="温度控制")
...
class ModelConfigOut(BaseModel):
    ...
    temperature: float = Field(..., description="温度控制")
    max_tokens: int = Field(..., description="限制单次回答的最大输出Token数")
    top_p: float = Field(..., description="核采样数(TOP-P)")
    top_k: int = Field(..., description="候选词数(TOP-K)")

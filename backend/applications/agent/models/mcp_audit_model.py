...
class McpAuditLog(ScaffoldModel, TimestampMixin):
    ...
    id = fields.CharField(
        default=unique_identify, max_length=64, primary_key=True, description="审计ID"
    )
    user_id = fields.BigIntField(db_index=True, description="用户ID")
    username = fields.CharField(max_length=64, default="", description="用户名")
    conversation_id = fields.CharField(max_length=64, null=True, db_index=True, description="对话ID")
    message_id = fields.IntField(null=True, description="消息ID")
    skill_id = fields.CharField(max_length=64, null=True, db_index=True, description="技能ID")
    skill_run_id = fields.CharField(max_length=64, null=True, db_index=True, description="SkillRun ID")
    mcp_server_id = fields.CharField(max_length=64, db_index=True, description="MCP服务ID")
    ...
    event_type = fields.CharField(max_length=32, db_index=True, description="tool_call|sampling|resource_read")
    tool_name = fields.CharField(max_length=128, db_index=True, description="工具或协议方法名")
    status = fields.CharField(max_length=16, db_index=True, description="done|error|cancelled")
    ...
    trace_id = fields.CharField(max_length=64, null=True, db_index=True, description="Trace ID")

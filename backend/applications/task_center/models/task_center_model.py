...
class TaskCenterInfo(ScaffoldModel, MaintainMixin, TimestampMixin, StateModel, ReserveFields):
    ...
    task_celery = fields.CharField(max_length=1024, null=True, description="Celery任务完全限定名")
    ...
    last_execute_state = fields.CharEnumField(TaskCenterStatus, default=None, null=True, description="最后执行状态")
    task_scheduler = fields.CharEnumField(TaskCenterScheduler, default=None, null=True, description="调度模式")
    ...
    class Meta:
        table = "keenrobot_task_center_info"
...
class TaskCenterRecord(ScaffoldModel, MaintainMixin, TimestampMixin, StateModel, ReserveFields):
    ...
    celery_status = fields.CharEnumField(TaskCenterStatus, default=TaskCenterStatus.RUNNING, description="调度状态")
    celery_scheduler = fields.CharEnumField(TaskCenterScheduler, default=None, null=True, description="调度方式")
